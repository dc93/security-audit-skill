...
>
> **Server-side web framework targets** (PHP forums/CMSes/community suites, Rails/Django/Flask/Node MVC monoliths — anything with a custom ORM or query builder, a template engine it compiles or evaluates, and a plugin/theme/extension system): use the deserialization, ORM-identifier, template-to-code, extension-as-code, and mass-assignment classes in [SERVER-SIDE-WEB-FRAMEWORK.md](SERVER-SIDE-WEB-FRAMEWORK.md) alongside the classes below (which includes worked stack instantiations for specific platforms — [IPS-COMMUNITY-NOTES.md](IPS-COMMUNITY-NOTES.md) for Invision Community / IPS, [VBULLETIN-NOTES.md](VBULLETIN-NOTES.md) for vBulletin, [MYBB-NOTES.md](MYBB-NOTES.md) for MyBB). When two versions of such a target are in scope at once (e.g. a legacy major line and its rewrite), also run the differential mode in [DIFFERENTIAL-AUDIT.md](DIFFERENTIAL-AUDIT.md).
